...
WriteLine(TillAmountSummary(cashTill));

// display the expected registerDailyStartingCash total
WriteLine($"Expected till value: {registerCheckTillTotal}");
WriteLine();
...
while (transactions > 0)
{
    ...
    WriteLine($"Customer is making a ${itemCost} purchase");
    WriteLine($"\t Using {paymentTwenties} twenty dollar bills");
    WriteLine($"\t Using {paymentTens} ten dollar bills");
    WriteLine($"\t Using {paymentFives} five dollar bills");
    WriteLine($"\t Using {paymentOnes} one dollar bills");
    ...
    {
        WriteLine($"Could not make transaction: {e.Message}");
    }
...
{
    WriteLine("The till currently has:");
    WriteLine($"{cashTill[3] * 20} in twenties");
    WriteLine($"{cashTill[2] * 10} in tens");
    WriteLine($"{cashTill[1] * 5} in fives");
    WriteLine($"{cashTill[0]} in ones");
    WriteLine();
}
