...
do
{
    // NOTE: the Clear method is throwing an exception in debug sessions
    Clear();
    ...
    WriteLine(" 2. Display all dogs with a specified characteristic");
    WriteLine();
        ...
        case "2":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 1].Contains("dog"))
                {
                    ...
                    foreach (string term in dogSearches)
                    {
                        ...
                        if (term != null && term.Trim() != "")
                        {
                            for (int j = 2; j > -1 ; j--)
                            {
                                ...
                                {
                                    Write($"\rsearching our dog {ourAnimals[i, 3]} for {term.Trim()} {icon} {j.ToString()}");
                                    Thread.Sleep(100);
                                }
                                
                                Write($"\r{new String(' ', BufferWidth)}");
                            }
                            ...
                            if (dogDescription.Contains(" " + term.Trim() + " "))
                            {
                                ...
                                WriteLine($"\rOur dog {ourAnimals[i, 3]} matches your search for {term.Trim()}");
                    ...
                    {
                        WriteLine($"\r{ourAnimals[i, 3]} ({ourAnimals[i, 0]})\n{dogDescription}\n");
                    }
                }
            }
            ...
            if (!matchesAnyDog)
            {
                WriteLine("None of our dogs are a match found for: " + dogCharacteristics);
            }
